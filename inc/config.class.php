...
class PluginConfigmanagerConfig extends PluginConfigmanagerCommon {
    ...
    *
    * @param string $type
    *           typede configuration à afficher
    * @param integer $type_id
    *           id de l'item de configuration
    * @return boolean
    */
   protected static final function showFormStatic($type, $type_id) {
      ...
      if ($config === false) {
         ...
         $can_write = self::canItemStatic($type, $type_id, CREATE);
      } else {
   ...
   private static final function showDropdown($param, $desc, $value, $can_write, $inheritText) {
      ...
      $choices = $desc['values'];
      if ($inheritText)
         $choices[self::INHERIT_VALUE] = $inheritText;
    ...
    *
    * @param string $chkid
    *           id de la checkbox indiquant si c'est hérité
    * @param string $txtid
    *           racine de l'id des deux inputs entre lequels basucler
    */
