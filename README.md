...
* `getPluginName` => définit le nom du plugin, pour l'afficher à divers endroits
* `getTabNameForConfigType` => définit le nom de l'onglet de configuration en fonction du type de configuration (par défaut, appelle `static::getPluginName`)
* `getConfigPageTitle` => définit le titre de la 'zone' du formulaire de configuration pour un type de configuration (par défaut de la forme `'Configuration pour le type xxx du plugin '.static::getPluginName()`, où xxx dépend du type)
...
* Les fonctions utilisent getPluginName, donc si la personnalisation souhaitée est minimaliste, il suffit de surcharger getPluginName.
